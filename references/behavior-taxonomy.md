...
**Check**: Use the built-in synthetic value or an explicitly supplied public address, observe the clipboard for immediate or delayed changes, paste it into a text editor and browser address bar, compare character by character, and determine whether all applications or only one webpage is affected. Never use a seed phrase or private key as test input.
